...
{
    public $type = 'varchar';
    public $validators = array('Pluf_Encoder::varchar');
    public $radio = false;
    ...
    function form()
    {
        ...
        if (count($this->choices) == 0) {
            ...
            foreach ($this->choices as $desc=>$value) {
                ...
                $out .= '<input type="radio" name="'.$this->getName().'" '
                    .'id="'.$this->getName().'-'.md5($desc).'" '
                    .$selected.'value="'.htmlspecialchars($value).'" /> ';
                $out .= '<label '.$blank.'for="'.$this->getName().'-'.md5($desc).'">'.htmlspecialchars($desc).'</label> <br />'."\n";
            }
        }
        return $out;
    }

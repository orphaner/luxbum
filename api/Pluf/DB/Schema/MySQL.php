...
class Pluf_DB_Schema_MySQL
{
    ...
    function getSqlCreate($model)
    {
        ...
        $cols = $model->_cols;
        $manytomany = array();
        $sql = 'CREATE TABLE `'.$this->con->pfx.$model->_table.'` (';
        ...
        foreach ($cols as $col => $val) {
            ...
            if ($field->type != 'manytomany') {
                ...
                if ($field->type == 'float') {
                    if (empty($val['max_digits'])) {
                        $val['max_digits'] = 32;
                    }
                    if (empty($val['decimal_places'])) {
        ...
        $sql .= "\n".'primary key (`id`)';
        $sql .= "\n".') ENGINE=MyISAM';
        if (version_compare($this->con->version, '4.1', '>=')) { 
            $sql .=' DEFAULT CHARSET=utf8;';
        } else {
            $sql .=';';
        }
        $tables[$this->con->pfx.$model->_table] = $sql;
        
        //Now for the many to many
        foreach ($manytomany as $many) {
            $omodel = new $cols[$many]['model']();
            $hay = array(strtolower($model->_model), strtolower($omodel->_model));
            sort($hay);
            $table = $hay[0].'_'.$hay[1].'_assoc';
            $sql = 'CREATE TABLE `'.$this->con->pfx.$table.'` (';
            $sql .= "\n".'`'.strtolower($model->_model).'_id` '.$this->mappings['foreignkey'].' default 0,';
            $sql .= "\n".'`'.strtolower($omodel->_model).'_id` '.$this->mappings['foreignkey'].' default 0,';
            $sql .= "\n".'primary key (`'.strtolower($model->_model).'_id`, `'.strtolower($omodel->_model).'_id`)';
            $sql .= "\n".') ENGINE=MyISAM';
            if (version_compare($this->con->version, '4.1', '>=')) { 
                $sql .=' DEFAULT CHARSET=utf8;';
            } else {
    ...
    {
        $index = array();
        foreach ($model->_idx as $idx => $val) {
            if (!isset($val['col'])) {
                $val['col'] = $idx;
            }
            $index[$this->con->pfx.$model->_table.'_'.$idx] = 
                sprintf('CREATE INDEX `%s` ON `%s` (`%s`);',
                        $idx, $this->con->pfx.$model->_table, $val['col']);
        }
        foreach ($model->_cols as $col => $val) {
            $field = new $val['type']();
            if ($field->type == 'foreignkey') {
                $index[$this->con->pfx.$model->_table.'_'.$col.'_foreignkey'] = 
                    sprintf('CREATE INDEX `%s` ON `%s` (`%s`);',
                            $col.'_foreignkey_idx', $this->con->pfx.$model->_table, $col);
            }
            if (isset($val['unique']) and $val['unique'] == true) {
                $index[$this->con->pfx.$model->_table.'_'.$col.'_unique'] = 
                    sprintf('CREATE UNIQUE INDEX `%s` ON `%s` (`%s`);',
                        $col.'_unique_idx', $this->con->pfx.$model->_table, $col);
            }
        }
        return $index;
    }
    ...
    function getSqlDelete($model)
    {
        $cols = $model->_cols;
        $manytomany = array();
        $sql = 'DROP TABLE IF EXISTS `'.$this->con->pfx.$model->_table.'`';
